...
export const MatchZodSchema = MatchBaseZodSchema.refine(
  (data) => {
    // PKが両方入力されている場合はPKゴールで判定
    if (data.home_pk_goal != null && data.away_pk_goal != null) {
      if (data.home_pk_goal > data.away_pk_goal) return data.result === "home";
      if (data.home_pk_goal < data.away_pk_goal) return data.result === "away";
...
export const MatchResponseSchema = MatchBaseZodSchema.omit({
  competition: true,
...
export const MatchPopulatedSchema = MatchBaseZodSchema.omit({
  competition: true,
  competition_stage: true,
  season: true,
  home_team: true,
