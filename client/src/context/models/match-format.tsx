...
type ContextModelType = ModelType.MATCH_FORMAT;
const ContextModelString = ModelType.MATCH_FORMAT;
type Form = MatchFormatForm;
type Get = MatchFormatGet;
type Model = MatchFormat;
...
const MatchFormatProvider = ({ children }: { children: ReactNode }) => {
  return <MetaCrudProvider>{children}</MetaCrudProvider>;
};

const useMatchFormat = () => {
  const metacrud = useContext(MatchFormatContext.metacrud);

  if (!metacrud) {
    throw new Error("useMatchFormat must be used within MatchFormatProvider");
  }
