...
import { Stadium, StadiumForm, StadiumGet } from "../../types/models/stadium";
...
type ContextModelType = ModelType.STADIUM;
const ContextModelString = ModelType.STADIUM;
type Form = StadiumForm;
type Get = StadiumGet;
type Model = Stadium;
...
const StadiumProvider = ({ children }: { children: ReactNode }) => {
  return (
    <SingleProvider>
      <BulkProvider>
...
const useStadium = () => {
  const single = useContext(StadiumContext.single);
  const bulk = useContext(StadiumContext.bulk);
  const metacrud = useContext(StadiumContext.metacrud);

  if (!single || !bulk || !metacrud) {
    throw new Error("useStadium must be used within StadiumProvider");
  }
