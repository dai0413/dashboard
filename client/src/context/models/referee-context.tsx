...
import { Referee, RefereeForm, RefereeGet } from "../../types/models/referee";
...
type ContextModelType = ModelType.REFEREE;
const ContextModelString = ModelType.REFEREE;
type Form = RefereeForm;
type Get = RefereeGet;
type Model = Referee;
...
const RefereeProvider = ({ children }: { children: ReactNode }) => {
  return (
    <SingleProvider>
      <BulkProvider>
...
const useReferee = () => {
  const single = useContext(RefereeContext.single);
  const bulk = useContext(RefereeContext.bulk);
  const metacrud = useContext(RefereeContext.metacrud);
