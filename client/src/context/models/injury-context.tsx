...
type ContextModelType = ModelType.INJURY;
const ContextModelString = ModelType.INJURY;
type Form = InjuryForm;
type Get = InjuryGet;
type Model = Injury;
...
const useInjury = () => {
  const single = useContext(InjuryContext.single);
  const bulk = useContext(InjuryContext.bulk);
  const metacrud = useContext(InjuryContext.metacrud);

  if (!single || !bulk || !metacrud) {
    throw new Error("useInjury must be used within InjuryProvider");
  }
