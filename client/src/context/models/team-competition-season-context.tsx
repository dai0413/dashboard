...
type ContextModelType = ModelType.TEAM_COMPETITION_SEASON;
const ContextModelString = ModelType.TEAM_COMPETITION_SEASON;
type Form = TeamCompetitionSeasonForm;
type Get = TeamCompetitionSeasonGet;
type Model = TeamCompetitionSeason;
...
const TeamCompetitionSeasonContext = {
  single: SingleContext,
  bulk: BulkContext,
  metacrud: MetaCrudContextContext,
};

const TeamCompetitionSeasonProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
...
const useTeamCompetitionSeason = () => {
  const single = useContext(TeamCompetitionSeasonContext.single);
  const bulk = useContext(TeamCompetitionSeasonContext.bulk);
  const metacrud = useContext(TeamCompetitionSeasonContext.metacrud);

  if (!single || !bulk || !metacrud) {
    throw new Error(
      "useTeamCompetitionSeason must be used within TeamCompetitionSeasonProvider"
    );
  }
